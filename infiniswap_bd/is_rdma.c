...
struct is_rdma_session {
	...
	struct is_remote_chunk_provider_observation provider_observation;
	bool has_provider_observation;
	...
	bool healthy;
	bool compatible;
	enum is_remote_chunk_provider_exclusion exclude_reason;
...
static void is_session_set_exclude(struct is_rdma_session *session,
				   enum is_remote_chunk_provider_exclusion reason);
...
static int is_session_record_provider_observation(
	...
	enum is_remote_chunk_provider_exclusion exclusion, u64 sequence)
{
...
static int is_session_record_unavailable_observation(
	struct is_rdma_session *session)
{
	enum is_remote_chunk_provider_exclusion exclusion = session->exclude_reason;

	if (!session->has_provider_observation)
		return -ENOENT;
	if (exclusion == IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_NONE)
		exclusion = session->compatible ?
			IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_UNHEALTHY :
			IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_CAPABILITY;
	return is_session_record_provider_observation(session,
		session->provider_observation.available_chunks, false, exclusion,
		session->next_request_id++);
}

static int is_session_repeat_provider_observation(
	struct is_rdma_session *session)
{
	enum is_remote_chunk_provider_observation_result result;
...
{
	return READ_ONCE(session->stopping) || session->failure_started ||
		!session->healthy || !session->compatible ||
				   ...
				   enum is_remote_chunk_provider_exclusion reason)
{
	...
	if (session->exclude_reason == IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_NONE)
...
static void is_fabric_fail_unstarted_provider(struct is_rdma_fabric *fabric,
	unsigned int provider_index)
{
	...
	if (provider_index < fabric->device->remote_chunk_provider_count)
		(void)is_remote_chunk_provider_failed(
			fabric->device->remote_chunks,
...
static void is_fabric_start_next_session(struct is_rdma_fabric *fabric)
{
	...
	if (!session) {
		...
		is_fabric_fail_unstarted_provider(fabric, index);
...
static void is_session_status_done(struct is_rdma_session *session)
{
	...
	if (!is_remote_only_session(session) || READ_ONCE(fabric->stopping) ||
...
static void is_mark_session_ready(struct is_rdma_session *session)
{
	...
	session->exclude_reason = IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_NONE;
	if (remote_only) {
...
static int is_request_remote_only_reservation(
	struct is_rdma_session *session,
	const struct is_remote_chunk_mapping_request *mapping)
{
	struct is_protocol_message *request = &session->outbound_message;
	int ret;

	if (!mapping ||
	    mapping->pool != IS_REMOTE_CHUNK_MAPPING_POOL_COMMITTED)
		return -EINVAL;
	session->mapping_claim = mapping->claim;
	is_init_message(session, request, IS_PROTOCOL_MSG_CHUNK_REQUEST,
		session->next_request_id++, false);
	request->payload.chunk_request.chunk_count = 1;
	request->payload.chunk_request.logical_start = mapping->logical_chunk;
			    ...
			    const struct is_protocol_message *status)
{
	...
	exclusion = !healthy ? IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_UNHEALTHY :
		(!available_chunks ? IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_ZERO_CAPACITY :
			       ...
			       const struct is_protocol_message *status)
{
	...
	ret = is_session_record_provider_observation(session, available_chunks,
		...
		!healthy ? IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_UNHEALTHY :
		(!available_chunks ? IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_ZERO_CAPACITY :
		 IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_NONE), status->header.request_id);
	if (ret)
		return ret;
	if (!healthy)
		return -EIO;
...
static void is_fabric_reservation_work(struct work_struct *work)
{
	...
	struct is_remote_chunk_mapping_request mapping = { 0 };
	struct is_rdma_session *session;
	...
	result = is_remote_chunk_next_mapping(device->remote_chunks, &mapping);
	switch (result) {
	case IS_REMOTE_CHUNK_NEXT_MAPPING_REQUEST:
		break;
	case IS_REMOTE_CHUNK_NEXT_MAPPING_COMPLETE:
		...
		for (index = 0; index < fabric->session_count; index++) {
			if (fabric->sessions[index])
				WRITE_ONCE(
					fabric->sessions[index]->reservation_complete,
					true);
		}
		is_fabric_complete_remote_only(fabric, 0);
		return;
	case IS_REMOTE_CHUNK_NEXT_MAPPING_PENDING_PROVIDER_FACTS:
	case IS_REMOTE_CHUNK_NEXT_MAPPING_CLAIM_ACTIVE:
		return;
	case IS_REMOTE_CHUNK_NEXT_MAPPING_INSUFFICIENT_CAPACITY:
	case IS_REMOTE_CHUNK_NEXT_MAPPING_NO_ELIGIBLE_CAPACITY:
		ret = -ENOSPC;
		break;
	case IS_REMOTE_CHUNK_NEXT_MAPPING_SHUTDOWN:
		ret = -ESHUTDOWN;
		break;
	default:
		ret = -EINVAL;
	...
	ret = is_request_remote_only_reservation(session, &mapping);
	mutex_unlock(&session->control_lock);
	if (!ret)
		return;
	if (ret == -EPROTO) {
	...
	for (index = 0; index < snapshot->provider_count; index++) {
		...
		if (provider->has_observation && !provider->failed &&
			...
			reason = is_remote_chunk_exclusion_name(provider->exclusion);
				...
				IS_REMOTE_CHUNK_PROVIDER_EXCLUDE_NONE)
			reason = is_remote_chunk_exclusion_name(
	...
	const struct is_remote_chunk_snapshot *snapshot, char *page)
{
	...
	for (index = 0; index < snapshot->provider_count; index++) {
		...
		unsigned int available = provider->diagnostic_available_chunks;
